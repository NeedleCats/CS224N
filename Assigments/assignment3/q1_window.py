...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        sentence_n = [start] * window_size + sentence + [end]*window_size
        windowed_data.extend(
            list(
                map(lambda inx:
                    (sum(sentence_n[inx: inx+window_size*2+1], []),
                     labels[inx]),
                    range(len(sentence)))))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable("W",
                             shape=[(self.config.n_window_features *
        ...
        z1 = tf.matmul(x, W)+b1
        h1 = tf.nn.relu(z1)
        h1_drop = tf.nn.dropout(h1, dropout_rate)
        U = tf.get_variable("U",
    ...
    def consolidate_predictions(self, examples_raw, examples, preds):
        """Batch the predictions in
        to groups of sentence length.
        """
        ret = []
        #pdb.set_trace()
        i = 0
