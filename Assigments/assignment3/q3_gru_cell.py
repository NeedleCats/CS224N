    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            b_z = tf.get_variable(
                ...
                initializer=initializer_class())
            z_t = tf.nn.sigmoid(
                tf.matmul(inputs, W_z) + tf.matmul(state, U_z) + b_z)
            ...
            b_r = tf.get_variable(
                ...
                initializer=initializer_class())
            r_t = tf.nn.sigmoid(
                tf.matmul(inputs, W_r) + tf.matmul(state, U_r) + b_r)
            W_o = tf.get_variable(
                name="W_o",
                shape=[self.input_size, self.state_size],
                initializer=initializer_class())
            ...
            b_o = tf.get_variable(
                ...
                initializer=initializer_class())
            o_t = tf.nn.tanh(
                tf.matmul(inputs, W_o) + r_t * tf.matmul(state, U_o) + b_o)
    ...
    with tf.Graph().as_default():
        with tf.variable_scope("test_gru_cell"):
            ...
            with tf.variable_scope("gru"):
                tf.get_variable(
                    "W_r",
                    initializer=np.array(np.eye(3, 2), dtype=np.float32))
                tf.get_variable(
                    "U_r",
                    initializer=np.array(np.eye(2, 2), dtype=np.float32))
                tf.get_variable(
                    "b_r", initializer=np.array(np.ones(2), dtype=np.float32))
                tf.get_variable(
                    "W_z",
                    initializer=np.array(np.eye(3, 2), dtype=np.float32))
                tf.get_variable(
                    ...
                    initializer=np.array(np.eye(2, 2), dtype=np.float32))
                tf.get_variable(
                    "b_z", initializer=np.array(np.ones(2), dtype=np.float32))
                tf.get_variable(
                    "W_o",
                    initializer=np.array(np.eye(3, 2), dtype=np.float32))
                tf.get_variable(
                    ...
                    initializer=np.array(np.eye(2, 2), dtype=np.float32))
                tf.get_variable(
                    "b_o", initializer=np.array(np.ones(2), dtype=np.float32))
            ...
            with tf.Session() as session:
                session.run(init)
                x = np.array([[0.4, 0.5, 0.6], [0.3, -0.2, -0.1]],
                             dtype=np.float32)
                h = np.array([[0.2, 0.5], [-0.3, -0.3]], dtype=np.float32)
                y = np.array([[0.320, 0.555], [-0.006, 0.020]],
                             dtype=np.float32)
                ht = y
                ...
                assert np.allclose(y_,
                                   ht_), "output and state should be equal."
                assert np.allclose(
                    ht, ht_,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Tests the GRU cell implemented as part of Q3 of Homework 3'
    )
