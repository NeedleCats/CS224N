...
class SequencePredictor(Model):
    def add_placeholders(self):
        ...
        self.inputs_placeholder = tf.placeholder(
            tf.float32, shape=[None, self.config.max_length, 1], name="x")
        self.labels_placeholder = tf.placeholder(
            tf.float32, shape=[None, 1], name="y")
    ...
    def add_training_op(self, loss):
        ...
        if self.config.clip_gradients:
            tmp_gradients = tf.clip_by_global_norm(
                gradients, clip_norm=self.config.max_grad_norm)[0]
            gradients = tmp_gradients

        grads_and_vars = [(gradients[i], variables[i])
...
Ur={:.2f}, Wr={:.2f}, br={:.2f}
Uz={:.2f}, Wz={:.2f}, bz={:.2f}
Uo={:.2f}, Wo={:.2f}, bo={:.2f}""".format(x, Ur[0, 0], Wr[0, 0], br[0],
                                          Uz[0, 0], Wz[0, 0], bz[0], Uo[0, 0],
                                          Wo[0, 0], bo[0]))
...
def compute_cell_dynamics(args):
    with tf.Graph().as_default():
        ...
        with tf.variable_scope("dynamics"):
            ...
            y_gru, h_gru = GRUCell(1, 1)(
                x_placeholder, h_placeholder, scope="cell")
            y_rnn, h_rnn = GRUCell(1, 1)(
                x_placeholder, h_placeholder, scope="cell")
...
def make_prediction_plot(args, losses, grad_norms):
    ...
    plt.subplot(2, 1, 2)
    plt.plot(
        np.arange(grad_norms.size), grad_norms.flatten(), label="Gradients")
    plt.ylabel("Gradients")
    plt.xlabel("Minibatch")
    output_path = "{}-{}clip-{}.png".format(
        args.output_prefix, "" if args.clip_gradients else "no", args.cell)
    plt.savefig(output_path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=
        'Runs a sequence model to test latching behavior of memory, e.g. 100000000 -> 1'
    )
    subparsers = parser.add_subparsers()

    command_parser = subparsers.add_parser(
        'predict', help='Plot prediction behavior of different cells')
    ...
    command_parser = subparsers.add_parser(
        'dynamics', help="Plot cell's dynamics")
    command_parser.add_argument(
        '-o',
        '--output-prefix',
        type=str,
