...
class Config:
    ...
    n_features = (
        2 * window_size +
        1) * n_word_features  # Number of features for every word in the input.
    ...
    def __init__(self, args):
        ...
        self.conll_output = self.output_path + "{}_predictions.conll".format(
            self.cell)
...
def pad_sequences(data, max_length):
    ...
    def fix(vector, length, default_value):
        if len(vector) < length:
            vector = vector + [default_value] * (length - len(vector))
        return vector[:length]

    for sentence, labels in data:
        ### YOUR CODE HERE (~4-6 lines)
        mask = [True] * len(sentence)
        sentence = fix(sentence, max_length, zero_vector)
        labels = fix(labels, max_length, zero_label)
        mask = fix(mask, max_length, False)
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.labels_placeholder = tf.placeholder(
            tf.int32, shape=(None, self.max_length))
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(emb_lookup, [
            -1, self.max_length,
        ...
        def featurize_windows(data, start, end, window_size=1):
            ...
            for sentence, labels in data:
                ...
                for window in window_iterator(
                        sentence, window_size, beg=start, end=end):
                    sentence_.append(sum(window, []))
                ret.append((sentence_, labels))
            return ret

        examples = featurize_windows(examples, self.helper.START,
                                     self.helper.END)
    ...
    def consolidate_predictions(self, examples_raw, examples, preds):
        ...
        for i, (sentence, labels) in enumerate(examples_raw):
            ...
            labels_ = [l for l, m in zip(preds[i], mask)
                       if m]  # only select elements of mask.
    ...
    def predict_on_batch(self, sess, inputs_batch, mask_batch):
        feed = self.create_feed_dict(
            inputs_batch=inputs_batch, mask_batch=mask_batch)
...
def test_pad_sequences():
    ...
    ret = [([[4, 1], [6, 0], [7, 0], [0, 0]], [1, 0, 0, 4],
            [True, True, True, False]),
           ([[3, 0], [3, 4], [4, 5], [5, 3]], [0, 1, 0, 2],
            [True, True, True, True])]

    ret_ = pad_sequences(data, 4)
    assert len(
        ret_
    ) == 2, "Did not process all examples: expected {} results, but got {}.".format(
        2, len(ret_))
    for i in range(2):
        assert len(
            ret_[i]
        ) == 3, "Did not populate return values corrected: expected {} items, but got {}.".format(
            3, len(ret_[i]))
        for j in range(3):
            assert ret_[i][j] == ret[i][
                j], "Expected {}, but got {} for {}-th entry of {}-th example".format(
