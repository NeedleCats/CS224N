...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    words_index = np.array([tokens[word] for word in sentence])
    vectors = wordVectors[words_index]
    sentVector = np.average(vectors, axis=0)
    ### END YOUR CODE
...
def getRegularizationValues():
    ...
    ### YOUR CODE HERE
    values = np.logspace(-6, 2, num=100, base=10)
    ### END YOUR CODE
...
def outputPredictions(dataset, features, labels, clf, filename):
    ...
    with open(filename, "w") as f:
        ...
        for i in xrange(len(dataset)):
            print >> f, "%d\t%d\t%s" % (labels[i], pred[i], " ".join(
                dataset[i][0]))
