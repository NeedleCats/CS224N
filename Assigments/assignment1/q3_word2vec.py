...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    v_c = predicted
    u_all = outputVectors
    y_hat_all = softmax(np.dot(u_all, v_c))
    cost = -np.log(y_hat_all[target])
    delta = y_hat_all.copy()
    delta[target] -= 1.0
    gradPred = np.dot(u_all.T, delta)
    grad = np.outer(delta, v_c)
    ### END YOUR CODE
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    z_c = sigmoid(np.dot(u_o, v_c))
    cost += -np.log(z_c)
    gradPred += (z_c - 1) * u_o
    grad[target] = (z_c - 1) * v_c

    for k_idx in indices[1:]:
        u_k = outputVectors[k_idx]
        z_k = sigmoid(np.dot(-u_k, v_c))
        cost += -np.log(z_k)
        gradPred += (z_k - 1) * (-u_k)
        grad[k_idx] += (z_k - 1) * (-v_c)
    ### END YOUR CODE
...
def skipgram(currentWord,
    ...
    for context in contextWords:
        context_idx = tokens[context]
        c_cost, c_grad_in, c_grad_out = \
            word2vecCostAndGradient(predicted,
                                    context_idx,
                                    outputVectors,
                                    dataset)
        cost += c_cost
        gradIn[current_idx] += c_grad_in
        gradOut += c_grad_out
    ### END YOUR CODE
...
def cbow(currentWord,
    ...
    ### YOUR CODE HERE
    predicted_idxs = [tokens[word] for word in contextWords]
    predicted = inputVectors[predicted_idxs].sum(axis=0)
    target = tokens[currentWord]
    c_cost, c_gradIn, c_gradOut = word2vecCostAndGradient(
