...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(
            looked_up, [-1, self.config.n_features * self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        z1 = tf.matmul(x, W1) + b1  # shape: self.config.hidden_size
        h1 = tf.nn.relu(z1)
    ...
    def run_epoch(self, sess, parser, train_examples, dev_set):
        ...
        for i, (train_x, train_y) in enumerate(
            ...
            # prog.update(
            #    i + 1, [("train loss", loss)], force=i + 1 == n_minibatches)
