...
class SoftmaxModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(
            tf.float32, [self.config.batch_size, self.config.n_features])
        self.output_placeholder = tf.placeholder(
            tf.int32, [self.config.batch_size, self.config.n_classes])
        ### END YOUR CODE
    ...
    def add_training_op(self, loss):
        ...
        """
        ### YOUR CODE HERE
        optimzer = tf.train.GradientDescentOptimizer(self.config.lr)
