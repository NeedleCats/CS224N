...
def softmax(x):
    ...
    x_exp = tf.exp(x_sub)
    sum_exp = tf.reduce_sum(x_exp, axis=1, keepdims=True)
...
def cross_entropy_loss(y, yhat):
    ...
    ### YOUR CODE HERE
    ln_y_hat = tf.log(yhat)
    tmp = tf.multiply(tf.to_float(y), ln_y_hat)
    score_sum = tf.reduce_sum(tmp)
    out = tf.negative(score_sum)
    ### END YOUR CODE
